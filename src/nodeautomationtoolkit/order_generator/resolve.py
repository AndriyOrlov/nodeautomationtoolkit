...
def record_from_document(facts: DocumentFacts, person: PersonMention) -> PersonRecord:
    ...
    current = facts.first("position_current") or (facts.positions[0] if facts.positions else "")
    # Посада призначення береться лише там, де документ прямо про неї каже
    # («на посаду …», «призначити …»): у рапорті друга згадка посади — не призначення.
    target = facts.first("position_target")
    if target == current:
        target = next((value for value in facts.all("position_target") if value != current), "")
    if current:
        record.current = Position(text=Value(current, DOCUMENT))
    if target:
        record.target = Position(
            text=Value(target, DOCUMENT),
            shpk=Value(facts.all("shpk")[-1] if facts.all("shpk") else "", DOCUMENT),
            vos=Value(facts.all("vos")[-1] if facts.all("vos") else "", DOCUMENT),
            tariff=Value(facts.all("tariff")[-1] if facts.all("tariff") else "", DOCUMENT),
        )
