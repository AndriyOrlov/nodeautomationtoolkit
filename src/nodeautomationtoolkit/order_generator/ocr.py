...
Tesseract і Poppler їдуть разом із програмою: тека `tesseract/` поруч із
`generate_extracts_qt.py` (tesseract.exe, бібліотеки, `tessdata/ukr.traineddata`,
`pdftoppm.exe` для PDF). Нічого встановлювати не треба.

Порядок пошуку: змінна `NAT_TESSERACT` → тека `tesseract/` програми → PATH →
звичайні місця встановлення. Якщо нічого немає — `OcrUnavailable` з підказкою.
"""
...
_INSTALL_HINT = (
    "Tesseract не знайдено. Він має лежати в теці «tesseract» поруч із програмою "
    "(tesseract.exe і tessdata/ukr.traineddata) або бути в PATH; шлях можна задати "
    "змінною середовища NAT_TESSERACT."
)


def bundled_folder() -> Path:
    """Тека «tesseract» поруч із програмою (корінь проєкту)."""
    return Path(__file__).resolve().parents[3] / "tesseract"


def _bundled(name: str) -> str | None:
    candidate = bundled_folder() / name
    return str(candidate) if candidate.is_file() else None


def tessdata_folder() -> str | None:
    """Тека з мовними файлами, якщо вона поруч із програмою."""
    from_env = os.environ.get("TESSDATA_PREFIX", "").strip('" ')
    if from_env and Path(from_env).is_dir():
        return from_env
    folder = bundled_folder() / "tessdata"
...
def find_tesseract() -> str | None:
    ...
    if from_env and Path(from_env).is_file():
        return from_env
    found = _bundled("tesseract.exe") or shutil.which("tesseract")
    if found:
        return found
...
def languages() -> list[str]:
    ...
    if not executable:
        return []
    try:
        output = _run([executable, "--list-langs", *_tessdata_args()])
...
def ocr_image(path: str | Path, language: str = DEFAULT_LANGUAGES) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="nat_ocr_") as folder:
        ...
        _run([executable, str(path), str(base), "-l", language, "--psm", "6", *_tessdata_args()])
...
def _pdf_pages(path: Path, folder: Path) -> list[Path]:
    ...
    pdftoppm = _bundled("pdftoppm.exe") or shutil.which("pdftoppm")
    if pdftoppm:
