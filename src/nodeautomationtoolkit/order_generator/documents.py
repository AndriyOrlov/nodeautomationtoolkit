...
from .ocr import IMAGE_SUFFIXES as OCR_SUFFIXES
from ..order_index.items import _PERSON_RE  # «ПРІЗВИЩЕ Ім'я По батькові,»

_UP = "А-ЯІЇЄҐ"
_LO = "а-яіїєґ"

#: Вид документа: (назва, слова, які мають бути ВСІ, слова, яких досить БУДЬ-ЯКОГО).
#: Перший збіжний рядок і перемагає, тому вужчі види стоять вище.
DOCUMENT_KINDS: tuple[tuple[str, tuple[str, ...], tuple[str, ...]], ...] = (
    ...
    ("подання до звільнення", ("подання",), ("до звільнення", "звільнити з військової служби")),
    ("подання до присвоєння звання", ("подання",), ("присвоєння військового звання",)),
    ("подання", (), ("подання",)),
    ("рапорт", (), ("рапорт", "заява", "бажаю звільнитися", "прошу звільнити", "прошу призначити")),
...
#: Дата народження — лише там, де вона позначена: «11.07.1988 р.н.», «дата народження: 11.07.1988».
_BIRTH_RE = re.compile(
    r"(\d{2}\.\d{2}\.(?:19|20)\d{2})\s*(?:р\.\s*н\.|року\s+народження)"
    r"|(?:дата\s+народження|народився|народилася)[:\s]*(\d{2}\.\d{2}\.(?:19|20)\d{2})"
    r"|(?<!\d)((?:19|20)\d{2})\s*р\.\s*н\.",
    re.IGNORECASE,
)
#: Місця, де може починатися назва посади всередині рядка.
_POSITION_STARTS = re.compile(
    r"(?:^|[\n;]|(?<=[,:])\s|на\s+посад[уи][:\s]+|посада[:\s]+|призначити\s+|перемістити\s+)", re.IGNORECASE
)
#: Ознака, що далі названо посаду, НА ЯКУ призначають.
_TARGET_HINT = re.compile(r"(на\s+посад|призначенн|призначити|перемістити|комплектуванню)", re.IGNORECASE)
...
#: Підстава із закону цілком: «підпункту «б» пункту 2 частини шостої статті 26 Закону …».
_LAW_REF_RE = re.compile(r"(?:під)?пункт[^.;]{0,90}?статт[іи]\s+\d+[^.;]{0,80}", re.IGNORECASE)
...
def read_text(path: str | Path, word_reader=None) -> str:
    """Текст будь-якого кадрового файла.

    .docx читається напряму, .doc/.rtf — через Word (`word_reader`),
    скани (.pdf, .png, .jpg …) — через Tesseract українською (`ocr.py`).
    """
    path = Path(path)
    suffix = path.suffix.casefold()
    if suffix == ".pdf" or suffix in OCR_SUFFIXES:
...
def detect_kind(text: str) -> str:
    ...
    for kind, required, any_of in DOCUMENT_KINDS:
        if all(marker.casefold() in head for marker in required) and (
            not any_of or any(marker.casefold() in head for marker in any_of)
        ):
            return kind
    return "невідомо"
...
def extract_facts(text: str, path: str = "", dictionary=None) -> DocumentFacts:
    ...
    for match in _LAW_REF_RE.finditer(text):
        ...
        break  # перша згадка найповніша: далі йдуть її частини

    found = _positions(text, dictionary)
    result.positions = [position for position, _offset in found]
    for position, offset in found:
        before = text[max(0, offset - 70):offset]
        field_name = "position_target" if _TARGET_HINT.search(before) else "position_current"
        _add(result.facts, field_name, position, before + position)
...
def _positions(text: str, dictionary) -> list[tuple[str, int]]:
    """Посади, які впізнав словник: (назва в називному, місце в тексті).

    Шукається не лише з початку рядка, а й після ком, двокрапок і слів
    «на посаду», «призначити» — у документах посада стоїть усередині речення.
    """
    ...
    seen: set[str] = set()
    for start in _POSITION_STARTS.finditer(text):
        offset = start.end()
        piece = text[offset:offset + 300].lstrip(" ,.;:-–—")
        offset += len(text[offset:offset + 300]) - len(piece)
        if len(piece) < 5:
            continue
        match = dictionary.match_at_start(piece)
        if match and match.nominative not in seen:
            seen.add(match.nominative)
            found.append((match.nominative, offset))
    return found
