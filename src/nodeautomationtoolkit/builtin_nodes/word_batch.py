...
def _try_create_extracts_via_word_com(
    ...
    # ⚠️ УВАГА: наприкінці функції використовується `DataTable`, якого тут
    # НЕ ІМПОРТОВАНО (локальні імпорти сусідніх функцій сюди не поширюються).
    # Через це гілка завжди падає `NameError`, збій ковтається, і витяги
    # насправді робить запасний шлях на python-docx.
    # Просто дописати імпорт НЕ МОЖНА: це вмикає мертву гілку, і
    # `test_signatory_slash_line_breaks` одразу падає — файл витягу
    # опиняється не там, де його чекають. Гілку треба або лагодити цілком
    # (разом із полями сторінки, розд. 12.2), або прибирати. Див.
    # `tests/test_no_undefined_names.py`, список KNOWN_UNFIXED.
    try:
        import win32com.client
    except ImportError:
        return None
