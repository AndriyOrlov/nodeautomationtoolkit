...
def test_ocr_reads_ukrainian_scan(tmp_path):
    """Скан українською читається вбудованим Tesseract (тека «tesseract» поруч із програмою)."""
    import pytest

    from nodeautomationtoolkit.order_generator import ocr

    if not ocr.is_available("ukr"):
        pytest.skip("немає Tesseract з українською мовою")
    from PIL import Image, ImageDraw, ImageFont

    image = Image.new("L", (1300, 130), 255)
    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype(r"C:\Windows\Fonts\times.ttf", 28)
    draw.text((20, 20), "Майора ПЕТРЕНКА Сергія Миколайовича, командира роти, 1234567890.", font=font, fill=0)
    scan = tmp_path / "скан.png"
    image.save(scan)

    from nodeautomationtoolkit.order_generator.documents import read_document

    facts = read_document(scan)
    assert [person.full_name for person in facts.people] == ["ПЕТРЕНКА Сергія Миколайовича"]
    assert facts.first("ipn") == "1234567890"
    assert "командир роти" in facts.positions
