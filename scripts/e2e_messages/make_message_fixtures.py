...
# 2.1 — внутрішнє переміщення в управлінні: у повідомлення НЕ переноситься
# (галочка на вкладці повідомлень, увімкнена за замовчуванням).
item(order, "2.1. Майора ВНУТРІШНЬЕНКА Олега Олеговича, старшого офіцера відділу "
            "планування управління зв'язку – СТАРШИМ ОФІЦЕРОМ ВІДДІЛУ ОРГАНІЗАЦІЇ "
            "УПРАВЛІННЯ ЗВ'ЯЗКУ.")
bio(order, "1986 р. н., освіта: академія державного управління у 2008 р.,", "у ЗС - із 08.2004.")
bio(order, "6789012345.")
para(order)
para(order)
...
# § 3 лише з внутрішнім переміщенням в управлінні — зникає разом зі шапкою.
para(order, "§ 3", C)
para(order)
para(order, "Відповідно до пункту 3 Положення про проходження військової служби "
            "з нижчепойменованими військовослужбовцями ПРИЗНАЧИТИ:", align=J)
para(order)
item(order, "4. Капітана ШТАБЕНКА Івана Івановича, офіцера відділу кадрів "
            "управління персоналу – СТАРШИМ ОФІЦЕРОМ ВІДДІЛУ КАДРІВ УПРАВЛІННЯ ПЕРСОНАЛУ.")
bio(order, "1991 р. н., освіта: ТВІ у 2013 р.,", "у ЗС - із 08.2009.")
bio(order, "7890123456.")
para(order)
para(order)
