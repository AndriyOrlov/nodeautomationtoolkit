...
# Пункти 2.1 і 4 — внутрішнє переміщення в управлінні: їх у змісті немає,
# а § 3, під яким не лишилось інших пунктів, зникає разом із ними.
for marker, why in (
    ("ВНУТРІШНЬЕНКА", "пункт 2.1 (внутрішнє переміщення в управлінні) потрапив у зміст"),
    ("ШТАБЕНКА", "пункт 4 (внутрішнє переміщення в управлінні) потрапив у зміст"),
    ("§ 3", "шапка § 3 без жодного пункту лишилась у змісті"),
    ("пункту 3 Положення", "шапка § 3 без жодного пункту лишилась у змісті"),
):
    if marker in body:
        problems.append(why)
for marker in ("ПРИКЛАДЕНКА", "ЗРАЗКОВА", "§ 2"):
    if marker not in body:
        problems.append(f"ВТРАТА ТЕКСТУ: зник «{marker}»")
